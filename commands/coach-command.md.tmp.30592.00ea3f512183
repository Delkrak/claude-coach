...
### 2. Check history
Read `.claude/tune/commands/<name>/history.md` if it exists. Note any recurring corrections (patterns that keep coming up) and what has been deliberately preserved — so those decisions are not revisited without reason.
...
Append an entry to `.claude/tune/commands/<name>/history.md` (create file and folders if needed) using this format:
